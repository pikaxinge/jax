...
for semantics in [gpu_core.LANExWG_SEMANTICS, gpu_core.LANExWARP_SEMANTICS]:
  mosaic_lowering_rules[semantics].update({
    ...
    lax.shift_left_p: partial(_binary_op_lowering_rule, impl=lambda x, y: x << y),
    lax.shift_right_logical_p: partial(_binary_op_lowering_rule, impl=lambda x, y: x._pointwise(arith_dialect.shrui, y, restrict_bitwidth=False)),
    lax.shift_right_arithmetic_p: partial(_binary_op_lowering_rule, impl=lambda x, y: x._pointwise(arith_dialect.shrsi, y, restrict_bitwidth=False)),
  })
